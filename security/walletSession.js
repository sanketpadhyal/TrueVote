export const getMetaMaskProvider = () => {
  ...
  if (eth.providers && Array.isArray(eth.providers)) {
    const realMetaMask = eth.providers.find(
      (p) => p.isMetaMask && !p.isPhantom && !p.isBraveWallet && !p.isCoinbaseWallet && !p.isTrust
    );
    if (realMetaMask) return realMetaMask;
    const fallbackMetaMask = eth.providers.find((p) => p.isMetaMask);
...
export const syncWalletSession = async (onDisconnect) => {
  if (typeof window === 'undefined') return;
...
export const setupWalletListeners = (onDisconnect, onAccountChange) => {
  if (typeof window === 'undefined') return () => {};
  const provider = getMetaMaskProvider();
