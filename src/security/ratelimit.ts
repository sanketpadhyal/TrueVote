export interface RateLimitStatus {
  allowed: boolean;
  reason?: string;
  remainingIpVotes: number;
  remainingDeviceVotes: number;
...
function saveTimestamps(key: string, list: number[]): void {
  if (typeof window === 'undefined') return;
  try {
...
export async function checkRateLimit(eventId?: string): Promise<RateLimitStatus> {
...
export async function recordSuccessfulVote(eventId?: string): Promise<void> {
  if (typeof window === 'undefined') return;
